...
in de woorden van de deelnemer. Tot slot `## Acties` als checklist.
...
De machinale laag heeft nu een eigen plek: `ceda-reflect` (in `cedanl/ceda-skills-library`,
`plugins/ceda-reflect/`) vangt correcties, toolfouten en afgewezen tool-calls en legt ze als
`agent-observaties-*.md` in dezelfde data-repo, met dezelfde frontmatter-sleutels. Beide
bestanden wijzen naar dezelfde commits, dus ze zijn achteraf samen te brengen.
