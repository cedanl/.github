...
The `[browser]` extra + the first `notebooklm login` download Chromium **automatically** (~170 MB, 30–90 s, no progress bar — wait it out). No manual `playwright install chromium` step is needed on macOS, Windows, or standard Linux.

- **Linux / CI:** if the browser fails to launch with missing-shared-library errors, install the system deps once: `playwright install-deps chromium` (needs sudo).
- **WSL:** auto-install works as normal; the login browser opens in the Windows host.
- **Chromium download fails with `does not support chromium on <os>`** (a distro newer than the bundled Playwright knows — e.g. Ubuntu 26.04): pin the platform to the nearest *supported* tag for the one-time download, then log in:
  ```bash
  # the -x64 arch suffix is mandatory — "ubuntu24.04" without it also fails
  PLAYWRIGHT_HOST_PLATFORM_OVERRIDE="ubuntu24.04-x64" python -m playwright install chromium
  notebooklm login
  ```
  The override is only needed for the Chromium *install*; normal runtime (`notebooklm login`, browser flows) needs no env var. uv-tool install? Call the bundled binary instead of `python -m playwright`, e.g. `~/.local/share/uv/tools/notebooklm-py/bin/playwright install chromium`.
